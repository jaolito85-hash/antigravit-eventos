...
| Dúvida que casa com ficha do guia com banner (line-up, cardápio) | Manda **só a imagem**, sem texto antes (pedido do Joao Marcos em 25/09). O texto da ficha fica para a triagem saber o que a arte contém. Relato Urgente, elogio e crítico nunca recebem a arte. Arte de bebida específica (cerveja, Red Bull, Coco Leve) sai com "Quer o cardápio completo? Responde QUERO" como **legenda da própria imagem**: em duas mensagens a Meta entregava o texto antes da foto. "Quero/sim/manda" com essa pergunta entre as 3 últimas falas do Tuca manda a arte do cardápio geral (`scope` `cardapio-geral`), sem triagem. |
| Resposta que fala do app ("baixe o aplicativo e veja a rota") | O link cadastrado no painel entra por código na linha de baixo (`server.com_link_do_app`), em qualquer urgência menos Crítico, que é texto fixo. O prompt continua proibindo mandar quem relata problema para o app; se a IA mencionar mesmo assim, o link vai junto. BaladApp é bilheteria e não conta como menção. Desde 25/09; antes a IA colava o link só às vezes. Quem perdeu algo ("perdi minha carteira") recebe uma carinha de empatia no começo, por instrução do prompt. |
| Pergunta em pedaços ("tem seda?" e depois "onde eu compro?", "quanto custa a cerveja?" e depois "e a água?") | As últimas 8 falas da pessoa e do Tuca (`historico_em_texto`, montado uma vez por mensagem em `worker._contexto`) vão para a **triagem** e para a resposta, como dado entre `<history>`. A triagem resolve a referência pela conversa: "onde eu compro?" vira "onde compro seda" e casa com a ficha da loja; o lugar onde a pessoa disse estar segue valendo para o setor. Referência curta aponta para o assunto **mais recente** (seda, "valeu", bebidas, "sabe onde compra?" é bebidas), e a resposta tira fato (lugar, preço) só do material oficial daquela resposta, nunca do histórico: em 25/09 a IA copiou a seda do histórico com a ficha das bebidas na mão. Tipo e urgência continuam saindo só da mensagem atual: "obrigado" depois de um problema é conversa. A triagem também devolve `continuacao`: true quando a mensagem só faz sentido com a conversa ("qto custa?", "e onde compro?"). Continuação com a arte já mandada nos últimos 30 minutos (`ARTE_RECENTE_MINUTOS`) **não repete a arte**: responde em texto a partir da ficha, e o histórico diz à IA qual arte já foi ("[mandou a arte: ...]"). Pergunta completa ("sabe se tem água de coco?") recebe a arte de novo, com a pergunta do cardápio, mesmo minutos depois: em 25/09 a regra só por tempo negou a arte a quem perguntou de novo 20 minutos depois. O simulador de `/tuca` manda as bolhas da tela como o mesmo histórico. Desde 25/09; antes só a resposta via a conversa e "onde eu compro?" abria chamado genérico com o link do app. |
